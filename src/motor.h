...
// Sets desired signed motor RPM setpoint for closed-loop control.
esp_err_t motor_set_target_rpm(float target_rpm);

// Convenience API to set signed target RPM and run one control update step.
esp_err_t motor_set_rpm(float target_rpm);

// Runs one closed-loop control update and adjusts bidirectional PWM outputs.
